...
public final class LocalVariableList extends FixedSizeList {
  ...
  public static LocalVariableList concat(LocalVariableList paramLocalVariableList1, LocalVariableList paramLocalVariableList2) {
    ...
    int k = paramLocalVariableList1.size();
    int m = paramLocalVariableList2.size();
    LocalVariableList localVariableList = new LocalVariableList(k + m);
    byte b = 0;
    int i = 0;
    while (true) {
      j = b;
      if (i < k) {
        localVariableList.set(i, paramLocalVariableList1.get(i));
        i++;
        continue;
      } 
      break;
    ...
    while (j < m) {
      localVariableList.set(k + j, paramLocalVariableList2.get(j));
      j++;
    } 
  ...
  public static LocalVariableList mergeDescriptorsAndSignatures(LocalVariableList paramLocalVariableList1, LocalVariableList paramLocalVariableList2) {
    ...
    LocalVariableList localVariableList = new LocalVariableList(j);
    for (int i = 0; i < j; i++) {
      Item item2 = paramLocalVariableList1.get(i);
      Item item3 = paramLocalVariableList2.itemToLocal(item2);
      Item item1 = item2;
      if (item3 != null)
        item1 = item2.withSignature(item3.getSignature()); 
      localVariableList.set(i, item1);
    } 
    localVariableList.setImmutable();
    return localVariableList;
  }
  ...
  public static class Item {
    ...
    public boolean matchesAllButType(Item param1Item) {
      return (this.startPc == param1Item.startPc && this.length == param1Item.length && this.index == param1Item.index && this.name.equals(param1Item.name));
    }
...
/* Location:              C:\Users\Student\Desktop\APK Decompiling\com.qualcomm.ftcdriverstation_38_apps.evozi.com\classes-dex2jar.jar!\org\firstinspires\ftc\robotcore\internal\android\dx\cf\code\LocalVariableList.class
