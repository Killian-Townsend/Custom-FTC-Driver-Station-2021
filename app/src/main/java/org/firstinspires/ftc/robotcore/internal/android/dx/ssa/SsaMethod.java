...
public final class SsaMethod {
  ...
  static BitSet bitSetFromLabelList(BasicBlockList paramBasicBlockList, IntList paramIntList) {
    ...
    for (int i = 0; i < j; i++)
      bitSet.set(paramBasicBlockList.indexOfLabel(paramIntList.get(i))); 
    return bitSet;
  }
    ...
    if (!this.backMode) {
      ...
      forEachInsn(new SsaInsn.Visitor() {
            private void addToUses(SsaInsn param1SsaInsn) {
              ...
              for (int i = 0; i < j; i++)
                SsaMethod.this.useList[registerSpecList.get(i).getReg()].add(param1SsaInsn); 
            }
      ...
      for (i = bool; i < this.registerCount; i++)
  ...
  private void convertRopToSsaBlocks(RopMethod paramRopMethod) {
    ...
    this.blocks = new ArrayList<SsaBasicBlock>(j + 2);
    int i;
    for (i = 0; i < j; i++) {
      SsaBasicBlock ssaBasicBlock = SsaBasicBlock.newFromRop(paramRopMethod, i, this);
  ...
  public static IntList indexListFromLabelList(BasicBlockList paramBasicBlockList, IntList paramIntList) {
    ...
    int j = paramIntList.size();
    for (int i = 0; i < j; i++)
      intList.add(paramBasicBlockList.indexOfLabel(paramIntList.get(i))); 
    return intList;
  }
  ...
  public int borrowSpareRegister(int paramInt) {
    int j = this.spareRegisterBase;
    int i = this.borrowedSpareRegisters;
    j += i;
    this.borrowedSpareRegisters = i + paramInt;
    this.registerCount = Math.max(this.registerCount, paramInt + j);
    return j;
  }
  ...
  public void computeReachability() {
    ...
    while (iterator.hasNext())
      ((SsaBasicBlock)iterator.next()).setReachable(0); 
  ...
  public void deleteInsns(Set<SsaInsn> paramSet) {
    for (SsaBasicBlock ssaBasicBlock : getBlocks()) {
      ...
      for (i = arrayList.size() - 1; i >= 0; i--) {
        ssaInsn = arrayList.get(i);
        if (paramSet.contains(ssaInsn)) {
  ...
  public int getCountReachableBlocks() {
    ...
    int i = 0;
    while (iterator.hasNext()) {
      if (((SsaBasicBlock)iterator.next()).isReachable())
        i++; 
    } 
    return i;
  }
  ...
  public SsaBasicBlock getExitBlock() {
    ...
    return (i < 0) ? null : this.blocks.get(i);
  }
  ...
  public ArrayList<SsaInsn>[] getUseListCopy() {
    ...
    for (int i = 0; i < this.registerCount; i++)
      arrayOfArrayList[i] = new ArrayList<SsaInsn>(this.useList[i]); 
...
/* Location:              C:\Users\Student\Desktop\APK Decompiling\com.qualcomm.ftcdriverstation_38_apps.evozi.com\classes-dex2jar.jar!\org\firstinspires\ftc\robotcore\internal\android\dx\ssa\SsaMethod.class
