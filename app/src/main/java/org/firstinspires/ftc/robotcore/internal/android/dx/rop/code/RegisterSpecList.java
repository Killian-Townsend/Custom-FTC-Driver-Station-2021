...
public final class RegisterSpecList extends FixedSizeList implements TypeList {
  ...
  public int getRegistersSize() {
    ...
    int j;
    for (j = 0; i < k; j = m) {
      RegisterSpec registerSpec = (RegisterSpec)get0(i);
      int m = j;
  ...
  public RegisterSpec specForRegister(int paramInt) {
    int j = size();
    for (int i = 0; i < j; i++) {
      RegisterSpec registerSpec = get(i);
  ...
  public RegisterSpecList withFirst(RegisterSpec paramRegisterSpec) {
    int j = size();
    RegisterSpecList registerSpecList = new RegisterSpecList(j + 1);
    for (int i = 0; i < j; i = k) {
      int k = i + 1;
      registerSpecList.set0(k, get0(i));
    } 
  ...
  public RegisterSpecList withOffset(int paramInt) {
    ...
    for (int i = 0; i < j; i++) {
      RegisterSpec registerSpec = (RegisterSpec)get0(i);
      if (registerSpec != null)
        registerSpecList.set0(i, registerSpec.withOffset(paramInt)); 
    } 
    if (isImmutable())
      registerSpecList.setImmutable(); 
    return registerSpecList;
  ...
  public RegisterSpecList withoutFirst() {
    ...
    for (int i = 0; i < j; i = k) {
      int k = i + 1;
      registerSpecList.set0(i, get0(k));
    } 
    if (isImmutable())
      registerSpecList.setImmutable(); 
    return registerSpecList;
  ...
  private static class Expander {
    ...
    private void expandRegister(int param1Int, RegisterSpec param1RegisterSpec) {
      ...
      if (bool) {
        registerSpec = param1RegisterSpec.withReg(this.base);
...
/* Location:              C:\Users\Student\Desktop\APK Decompiling\com.qualcomm.ftcdriverstation_38_apps.evozi.com\classes-dex2jar.jar!\org\firstinspires\ftc\robotcore\internal\android\dx\rop\code\RegisterSpecList.class
